...
def billing_plans():
    return {"ok": True, "plans": quota.plan_catalog(), "currency": "RUB"}
...
def billing_admin_activate(request: FastAPIRequest, payload: PlanActivationRequest):
    ...
    try:
        state = quota.set_plan(payload.user_id, payload.plan, payload.status, payload.period_end)
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from exc
...
def agents_invoke(request: AgentRequest):
    try:
        result = invoke_agent(request.agent, request.message, history=request.history, context=request.context, user_id=security.current_user_id())
        return result
