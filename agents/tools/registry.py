...
_TOOLS = {
    ...
    "music.diagnose_vocal_in_section": ToolSpec(
        ...
        description="Диагностирует, почему вокал может теряться в секции. Если время не задано, автоматически выбирает вероятный chorus/drop или секцию по section_name.",
        input_schema={
            "type": "object",
            "properties": {
                "section_start_sec": {"type": "number", "minimum": 0},
                "section_end_sec": {"type": "number", "exclusiveMinimum": 0},
                "section_name": {"type": "string", "minLength": 1, "maxLength": 40},
            },
            "required": [],
    ...
    "music.analyze_mix": ToolSpec(
        name="music.analyze_mix", description="Структурирует результаты анализа микса, проблемы и приоритеты обработки.",
        input_schema={"type": "object", "properties": {"analysis": {"type": "object"}, "decisions": {"type": "array", "items": {"type": "object"}}, "master_report": {"type": "object"}}, "required": ["analysis", "decisions", "master_report"], "additionalProperties": False}, handler=analyze_mix,
    ),
    "music.build_advice": ToolSpec(
        name="music.build_advice", description="Формирует локальные рекомендации по миксу без вызова LLM.",
        input_schema={"type": "object", "properties": {"analysis": {"type": "object"}, "decisions": {"type": "array", "items": {"type": "object"}}, "master_report": {"type": "object"}}, "required": ["analysis", "decisions", "master_report"], "additionalProperties": False}, handler=build_mix_advice,
    ),
}
