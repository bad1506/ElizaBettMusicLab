...
function syncPageFromUrl() { const path = window.location.pathname.replace(/\/+$/, '') || '/'; const page = ROUTES[path] || 'home'; const labels = Object.entries(PAGE_BY_LABEL).filter(([, route]) => route === path).map(([label]) => label); const button = Array.from(document.querySelectorAll('button')).find(candidate => labels.includes(candidate.textContent?.trim() || '')); button?.click(); document.title = page === 'home' ? 'SØVA — Music Intelligence' : `${page === 'tools' ? 'AI Music Tools' : page[0].toUpperCase() + page.slice(1)} · SØVA` }
