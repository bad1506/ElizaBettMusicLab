...
const nav = [
  ...
  ["Проекты", "projects"], ["Тарифы", "pricing"], ["О SØNA", "about"],
] as const;
...
function normalizeChart(payload: any): ChartTrack[] {
  const root = payload?.result || payload;
  const raw = root?.chart || root?.items || root?.tracks || root?.playlist?.tracks || [];
  if (!Array.isArray(raw)) return [];
  return raw.slice(0, 20).map((entry: any, index: number) => {
    const row = entry?.track || entry?.track_short || entry;
    const artists = row?.artists || row?.artist || [];
    const artist = Array.isArray(artists) ? artists.map((a: any) => a?.name).filter(Boolean).join(", ") : String(artists?.name || artists || "");
    const coverUri = row?.coverUri || row?.cover_uri || row?.album?.coverUri || row?.album?.cover_uri;
...
const projects = [
  { name: "ВСЁ РАВНО", type: "Single · Pop", status: "MASTERED", meta: "2:48 · 2026", accent: "pink" },
  { name: "Тише", type: "Single · Acoustic Pop", status: "IN PRODUCTION", meta: "Demo → Master", accent: "blue" },
