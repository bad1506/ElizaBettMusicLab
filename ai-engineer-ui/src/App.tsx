...
export default function App() {
  ...
  async function readError(res: Response) {
    ...
    try { const data = JSON.parse(text); return data.detail || data.message || text; } catch { return text || `HTTP ${res.status}`; }
  }
  ...
  function selectAudio(next?: File) {
    ...
    if (!EXTENSIONS.includes(ext)) { add({ role: "assistant", text: `Формат .${ext || "unknown"} не поддерживается. Используй WAV, MP3, FLAC, M4A или OGG.` }); return; }
    if (next.size > MAX_BYTES) { add({ role: "assistant", text: "Файл слишком большой. Максимальный размер — 100 MB." }); return; }
    setFile(next);
    add({ role: "assistant", text: `Файл «${next.name}» принят. Теперь скажи, какой результат нужен.` });
  }

  async function uploadAudio() {
    if (!file) throw new Error("Сначала загрузи аудиофайл.");
  ...
  function context() {
    return messages[tool].slice(-8).map((m) => `${m.role === "user" ? "Клиент" : "AI"}: ${m.text}`).join("\n");
  }

  async function send() {
    const text = input.trim();
    if (!text || busy) return;
    add({ role: "user", text }); setInput(""); setBusy(true);
    try {
      if (tool === "songwriter") {
        const res = await fetch(`${API}/songwriter`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ request: `${context()}\nКлиентская задача: ${text}`, mode: "SONG" }) });
        if (!res.ok) throw new Error(await readError(res)); const data = await res.json(); add({ role: "assistant", text: data.answer || "Готово." });
      } else if (tool === "trends") {
        const res = await fetch(`${API}/songwriter/trends`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ focus: `${context()}\nНовая задача: ${text}` }) });
        ...
        const data = await uploadAudio(); add({ role: "assistant", text: formatAnalysis(data.analysis || data) });
      } else if (tool === "master") {
        await uploadAudio();
        const lufs = text.match(/-?\d+(?:[.,]\d+)?\s*LUFS/i)?.[0]?.replace(",", ".");
        const ceiling = text.match(/(?:ceiling|пик|потолок)[^\d-]*(-?\d+(?:[.,]\d+)?)/i)?.[1]?.replace(",", ".");
        const intensity = /плот|агрессив|громк/i.test(text) ? "aggressive" : /мяг|динами|натурал/i.test(text) ? "gentle" : "balanced";
        const res = await fetch(`${API}/master`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ target_lufs: lufs ? Number(lufs) : -10.5, ceiling_db: ceiling ? Number(ceiling) : -1, intensity, profile: "suno6_commercial" }) });
        if (!res.ok) throw new Error(await readError(res)); const data = await res.json(); add({ role: "assistant", text: `Мастеринг завершён.\n\nTarget: ${data.final_analysis?.lufs ?? "—"} LUFS\nПрофиль: ${intensity}\nCeiling: ${ceiling ? `${ceiling} dB` : "-1 dB"}\n\nЕсли хочешь другой характер — напиши, например: «сделай мягче», «сделай плотнее» или «дай -9 LUFS».` });
      }
    } catch (e) { add({ role: "assistant", text: `Не удалось выполнить запрос: ${String(e).replace(/^Error:\s*/, "")}` }); }
