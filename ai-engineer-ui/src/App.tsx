...
export default function App() {
  ...
  return <div className="site-shell">
    <header className="site-header">
      ...
      <div className="header-actions"><button className="language"><Globe2 size={16} /> RU <ChevronDown size={12} /></button><button className="telegram" onClick={() => window.open("https://t.me/", "_blank")}>Открыть в Telegram <ArrowRight size={15} /></button></div>
...
function Home({ go }: { go: (page: string) => void }) {
  return <main>
    ...
    <section className="telegram-banner"><div className="banner-copy"><span className="banner-kicker">✦</span><h3>Твоя музыка.<br />Без границ.</h3><p>Профессиональные AI-инструменты теперь доступны в Telegram.</p><button className="dark-button" onClick={() => window.open("https://t.me/", "_blank")}>Открыть в Telegram <ArrowRight size={15} /></button></div><div className="banner-image"></div><div className="mini-app"><span>TELEGRAM</span><h3>Music Lab<br />Mini App</h3><p>Создавай музыку прямо в Telegram. Всегда с тобой.</p><button onClick={() => window.open("https://t.me/", "_blank")}>Запустить Mini App <ArrowRight size={14} /></button></div></section>
