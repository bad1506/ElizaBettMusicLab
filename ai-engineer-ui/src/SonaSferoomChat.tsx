...
const QUICK_ACTIONS = [
  ...
  ["АНАЛИЗ АУДИО", "Разбери мой трек: микс, динамика, LUFS и точки улучшения", "audio-analysis"],
  ["МАСТЕРИНГ", "Подскажи, как сделать коммерческий мастер для этого трека", "mastering"],
  ["ТРЕНДЫ", "Найди актуальные музыкальные и social-тренды и преврати их в идеи", "trends"],
  ["СТАТИСТИКА", "Разбери мою статистику и скажи, на что обратить внимание", "analytics"],
  ["ФИНАНСЫ", "Помоги разобраться с финансами музыкального проекта", "finance"],
  ["ПРОЕКТ", "Помоги спланировать мой музыкальный проект от идеи до релиза", "projects"],
...
export default function SonaSferoomChat() {
  ...
  async function sendText(value = input, selectedSkill = skill) { const text = value.trim(); if (!text || busy) return; const history = messages.slice(-20); setMessages(prev => [...prev, { role: "user", text }]); setInput(""); setBusy(true); try { const response = await fetch("/api/sona-chat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: text, history, context: { product: "SØNA", language: "ru", client: "web", interface: "sferoom", skill: selectedSkill } }) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data?.detail || "Не удалось получить ответ"); setMessages(prev => [...prev, { role: "assistant", text: data.answer || "Не удалось получить ответ." }]); } catch (error) { setMessages(prev => [...prev, { role: "assistant", text: error instanceof Error ? `Не удалось ответить: ${error.message}` : "Не удалось ответить. Попробуй ещё раз." }]); } finally { setBusy(false); } }
