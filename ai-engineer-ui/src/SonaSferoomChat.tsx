...
type Message = { role: "assistant" | "user"; text: string; meta?: { agent?: string; skill?: string; tools?: string[] } };
...
export default function SonaSferoomChat() {
  ...
  async function sendText(value = input, selectedAgent = agent) {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: "assistant", text: data.answer || "Не удалось получить ответ.", meta: { agent: data.agent, skill: data.skill, tools } }]);
