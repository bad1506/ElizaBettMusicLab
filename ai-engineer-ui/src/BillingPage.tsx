...
export default function BillingPage() {
  ...
  async function load() {
    ...
    try {
      const [planData, usageData] = await Promise.all([api("/billing/plans"), api("/billing/usage")]);
      setPlans(planData.plans || {}); setUsage(usageData.usage || null);
  ...
  return <div className="billing-page">
    ...
    <main className="billing-main">
      ...
      <section className="billing-grid">
        {loading && !order.length ? <div className="billing-loading"><RefreshCw className="spin"/> Загружаем тарифы…</div> : order.map(key => { const p = plans[key]; const features = Object.entries(p.features || {}); const isCurrent = current === key; return <article className={`billing-card ${key === "pro" ? "featured" : ""}`} key={key}>
          {key === "pro" && <div className="billing-badge">SØNA PRO</div>}
          <span className="billing-eyebrow">{key.toUpperCase()}</span><h2>{p.name || key}</h2><div className="billing-price">{key === "free" ? "0 ₽" : <>{rub(p.price_rub)}<small>/мес</small></>}</div>
          <div className="billing-current">{isCurrent ? "Текущий тариф" : key === "free" ? "Для знакомства" : "Для регулярной работы"}</div>
          <ul>{features.map(([name, value]) => <li key={name}><Check size={14}/><span>{prettyFeature(name)}</span><b>{value.limit}</b></li>)}</ul>
          {p.storage_mb ? <div className="billing-storage">Хранилище: {new Intl.NumberFormat("ru-RU").format(p.storage_mb)} MB</div> : null}
      ...
      {paid && <div className="billing-note">Тариф активирован для этого аккаунта. Лимиты обновляются автоматически с началом нового периода.</div>}
    </main>
  </div>;
}
